...
public class Questionnaire {
   ...
   //private String mängija_nimi; // Mängija nimi
    ...
    public Questionnaire(String input) throws Exception {
    ...
    private void alustaMängu() throws InterruptedException {
        ...
        if(protsent <= 50){
        	System.out.println("Tundub, et peaksite õppima. Teie tulemus on:" + protsent +"%");}
        else if (protsent > 50 && protsent <= 85){
        	System.out.println("See tulemus on küllaltki hea, ent siiski tuleb veel harjutada! Saite " +protsent+ "%");
        }
        else
        	System.out.println("Väga tubli. Nii õppides võite isegi füüsikuks saada. Tulemus:" + protsent+"%");
        System.out.println();
        scanner.close();
        
        System.out.println("---------------------LÄBIVAATUS---------------------");
        
        // Lõpptulemus
        for (Küsimus küsimus: küsitud) {
            if (küsimus.onÕige())
                System.out.println(küsimus.toString()+"✔"); 
            else
                System.out.println(küsimus.toString()+"✘"); 
        }
        }
